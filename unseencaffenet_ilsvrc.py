            ...
            for img_id in range(num_test):
                ...
                if use_noise==True :
                    ...
                    if args.attack=='FFF':
                        ...
                        adv_img = np.transpose(adv_img[0, 0:224, 0:224, :], (2, 0, 1))
                        img_temp += adv_img[[2, 1, 0], :, :]
                        ...
                        # switch below line file with fc6 layer attack 'Attack_samples/SFool/alexnet_uap_5_fc6.h5'
                        # and average final results to get reported restoration accuracy in paper
...
parser.add_argument('--attack', help='Select unseen attack', default='FFF',choices=['FFF','NAG','SFool'])
...
if not args.load:
    ...
    if args.defense=='True':
        model_proto = 'Prototxt/CaffeNet/deploy_caffenet_FRU.prototxt'
    else:
        model_proto = 'Prototxt/CaffeNet/deploy_caffenet.prototxt'
